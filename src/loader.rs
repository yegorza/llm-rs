...
pub fn load_llama(path: &str) -> Model {
    ...
    for i in 0..22 {
        ...
        let block = TransformerBlock {
            ...
            ln_2_bias: None,
            q_proj: Some(load_tensor(&file_data, &header, header_size, &format!("{}.self_attn.q_proj.weight", prefix)).transpose()),
            k_proj: Some(load_tensor(&file_data, &header, header_size, &format!("{}.self_attn.k_proj.weight", prefix)).transpose()),
            v_proj: Some(load_tensor(&file_data, &header, header_size, &format!("{}.self_attn.v_proj.weight", prefix)).transpose()),
            c_proj_weight: load_tensor(&file_data, &header, header_size, &format!("{}.self_attn.o_proj.weight", prefix)).transpose(),
            c_proj_bias: None,
            gate_proj: Some(load_tensor(&file_data, &header, header_size, &format!("{}.mlp.gate_proj.weight", prefix)).transpose()),
            up_proj: Some(load_tensor(&file_data, &header, header_size, &format!("{}.mlp.up_proj.weight", prefix)).transpose()),
            down_proj: Some(load_tensor(&file_data, &header, header_size, &format!("{}.mlp.down_proj.weight", prefix)).transpose()),
            c_attn_weight: None,
            c_attn_bias: None,
            mlp_fc_weight: None,
            mlp_fc_bias: None,
    ...
    Model {
        ...
        ln_f_weight,
        ln_f_bias: None,
        lm_head: Some(load_tensor(&file_data, &header, header_size, "lm_head.weight").transpose()),
    }
}
